...
# -------------------------
# Cargar scaler y modelo cardiaco
# -------------------------
...
def _intensity_note(level):
    if level == 0: return "Inicia suave: 2–3 sesiones/semana."
    if level == 1: return "Mantén 3–4 sesiones/semana."
    return "Planifica 4–5 sesiones/semana."


def _build_heart_features(age, gender_str, bmi, activity_0_4, cols):
    sex = 'Male' if gender_str.lower().startswith('m') else 'Female'
    age_cat = (
        '18-24' if age < 25 else
        '25-29' if age < 30 else
        '30-34' if age < 35 else
        '35-39' if age < 40 else
        '40-44' if age < 45 else
        '45-49' if age < 50 else
        '50-54' if age < 55 else
        '55-59' if age < 60 else
        '60-64' if age < 65 else
        '65-69' if age < 70 else
        '70-74' if age < 75 else
        '75-79' if age < 80 else '80 or older'
    )
...
def recommend_full(gender_str, age, height_cm, weight_kg, activity_0_4, goal_str):
    ...
    bmr = 10*weight_kg + 6.25*height_cm - 5*age + (5 if gender_str.lower().startswith("m") else -161)
    PAL = [1.2, 1.375, 1.55, 1.725, 1.9][activity_0_4]
    tdee = bmr * PAL

    # calorías según objetivo
    goal_adj = {'fat_burn': -400, 'maintain': 0, 'muscle_gain': +400}.get(goal_str, 0)
    kcal = tdee + goal_adj

    gender_num = 0 if gender_str.lower().startswith("m") else 1
