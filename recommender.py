...
def anfis_predict(interpreter, x):
    """Ejecuta predicción TFLite ANFIS"""
...
def _intensity_note(level):
    if level == 0:
        return "Inicia suave: 2–3 sesiones/semana (movilidad, caminar, core básico)."
    if level == 1:
        return "Mantén 3–4 sesiones/semana (cardio moderado + fuerza ligera)."
...
def recommend_full(gender_str, age, height_cm, weight_kg, activity_0_4, goal_str):
    ...
    bmr = 10 * weight_kg + 6.25 * height_cm - 5 * age + (
        5 if gender_str.lower()[0] == "m" else -161
    )
